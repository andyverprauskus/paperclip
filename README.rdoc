...
I wanted to be able to declaratively specify the locale file system for the test environment and S3 for development and production. Doing something similar to config/s3.yml seemed like the trick. That's implemented in this fork.

  has_attached_file optionally takes a :config => file_path_string argument

which should point to a yaml file with separate sections for :test, :development, and :production, (and whatever other rails envs you use).  These can have any of the arguments that are normally specified to has_attached_file and they're overridden by any args you explicitly make. 

Below is my config/paperclip.yml. It's formatted to look good in RDoc. Remove two spaces from each line if you use it to start your config
